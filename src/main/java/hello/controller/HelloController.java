...
/**
 * REST controller that handles HTTP requests for date/time operations, string operations,
 * and file operations using Java 8 features.
 */
@RestController
public class HelloController {

    String joinTemplate = "Joining All String ID's with JOIN method: ";
    String makeDistinctAndSortCharactersTemplate = "-------------Get all ID characters, select " +
            "distict and sort with ID=   ";
    String splitAllIdWithColonSelectIDWithJavaKeywordThenSortThenJoinTemplate = 
            "-------------Split All Id With Colon," +
            "Select ID With \"Java\" Keyword," +
            " Then Sort Then Join ";
    String findIdHavingCharacterTemplate = "-------------Return All ID having character " +
            "\'g\' in it:  ";
    String findAllFilesInPathAndSortTemplate = "---------Find all files in path and sort:    ";
    String findParticularFileInPathAndSortTemplate = "----------Find File in present directory " +
            "which strats with \"grad\",provided maximum depth=25 and sort : ";
    String findParticularFileInPathAndSortWithWalkFunctionTemplate = "----------Find File in " +
            "present directory which strats with \"grad\",provided maximum depth=25 and sort :  " +
            "with walk function";
    String readFileWithStreamFunctionTemplate = "---------Read \"temp.txt\" file with stream " +
            "functions, having \"print\" witin it:  ";
    ...
    public String index() {
        ...
        return "Greetings from Spring Boot! ----------------------" +
                ...
                "----------------------" +
                "Datetime tomorrow will be " + 
                String.valueOf(myTimeClient.getLocalDateTime().plusDays(1)) + 
                "----------------------" +
                "Datetime of previous month was " + 
                String.valueOf(myTimeClient.getLocalDateTime().minus(1, ChronoUnit.MONTHS)) + 
                "----------------------" +
                ...
                "-------------------" +
                "Time in California: " + 
                myTimeClient.getZonedDateTime("Canada/Central").toString();
    }
    ...
    public String showFileOperation() {
        ...
        String findParticularFileInPathAndSortWithWalkFunction = 
